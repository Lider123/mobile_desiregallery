...
class StorageHelper(private val storage: FirebaseStorage) : IStorageHelper {
    ...
    override fun uploadProfileImage(bitmap: Bitmap, userLogin: String, callback: IStorageHelper.Callback) {
        uploadImage(bitmap, "${STORAGE_PROFILE_IMAGES_DIR}/${userLogin}.jpg", callback)
    }

    override fun uploadPostImage(bitmap: Bitmap, postId: String, callback: IStorageHelper.Callback) {
