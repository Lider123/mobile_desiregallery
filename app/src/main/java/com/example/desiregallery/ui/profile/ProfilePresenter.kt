...
class ProfilePresenter(private val view: IProfileContract.View) : IProfileContract.Presenter, KoinComponent {
        ...
        if (account is EmailAccount) {
            ...
            firebaseUser?.updateProfile(profileUpdates)?.addOnCompleteListener(activity) { task ->
                if (task.isSuccessful)
                    logInfo(TAG, "Data of user ${emailUser.login} have successfully been saved to firebase auth")
                else
    ...
    override fun uploadPhoto(imageUri: Uri, resolver: ContentResolver, onFailure: () -> Unit,
                             onComplete: () -> Unit) {
        ...
        storageHelper.uploadProfileImage(selectedImage, emailUser.login,
            object: IStorageHelper.Callback {
